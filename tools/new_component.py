...
Creates components/<group>/<name>/ with component.json, README.md and, for a skill, SKILL.md; for python and agents
(Python too), a tests/ folder with one passing test so the shelf's --test run is green from the first commit; for an
agent, TEMPLATE.md and the manifest's `agent` block.
"""
...
def main() -> int:
    ...
    ap.add_argument("group", choices=("python", "typescript", "skills", "agents"))
    ...
    os.makedirs(dest)
    language = {"python": "python", "typescript": "typescript", "skills": "markdown", "agents": "python"}[a.group]
    test = {"python": "python3 -m unittest discover -s tests -t . -v", "typescript": "npm ci --no-audit --no-fund && npx vitest run", "markdown": ""}[language]
    manifest = {"name": a.name, "version": "0.1.0", "category": a.category, "language": language, "summary": a.summary, "status": "draft", "signoff": {"owner": None, "ai_security": None}, "used_in": [],
                ...
                "example": {"path": {"python": "example.py", "typescript": "example.ts", "markdown": "EXAMPLE.md"}[language], "run": {"python": "python3 example.py", "typescript": "npx tsx example.ts", "markdown": ""}[language]}, "vendored": [],
    ...
    if a.category == "agent":
        # Set before the manifest is written: an agent declares its template, tools and harness in `agent`.
