...
REQUIRED = ("name", "kind", "language", "summary", "status", "source", "owner", "tags", "test", "spec")
...
def render(ms: list[dict]) -> str:
    ...
    for kind in KINDS:
        ...
        lines += [f"## {KIND_LABEL[kind]}", "", "| Component | Language | Status | Summary | From | Implements (spec §) | Tags |", "| --- | --- | --- | --- | --- | --- | --- |"]
        for m in group:
            src = m["source"].get("project", "")
            if m["source"].get("path"): src += f" (`{m['source']['path']}`)"
            lines.append(f"| [{m['name']}]({m['_rel']}/) | {m['language']} | {m['status']} | {m['summary']} | {src} | {spec_short(m)} | {', '.join(m['tags'])} |")
        lines.append("")
    lines += ["## By tag", ""]
