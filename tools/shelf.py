...
REQUIRED = ("name", "kind", "language", "summary", "status", "source", "owner", "tags", "test", "spec", "version", "signoff", "walkthrough", "example")
...
def hub_listing(m: dict) -> dict:
    ...
    return {"id": m["name"], "slug": m["name"], "kind": HUB_KIND[m["kind"]], "name": title_of(m), "description": m["summary"], "road": "R1",
            "lifecycle": HUB_LIFECYCLE[m["status"]] if signed_state(m) == "signed" else ("retired" if m["status"] == "deprecated" else "preview"), "icon": "m" if "security" in m["tags"] else "", "glyph": "shield" if "security" in m["tags"] else HUB_GLYPH[m["kind"]],
            "meta": [{"text": f"R1 · {m['kind']}", "kind": ""}, {"text": m["language"], "kind": "mono"}, {"text": f"v{m['version']}", "kind": "mono"}, {"text": "signed" if signed_state(m) == "signed" else "sign-off pending", "kind": "ok" if signed_state(m) == "signed" else "warn"}],
...
def hub_detail(m: dict, listing: dict) -> dict:
    ...
    return {**listing, "crumbs": ["Discover", "Knowledge" if listing["kind"] == "knowledge" else "Tools", listing["name"]], "youActAt": "L1", "ladderMax": "L1",
            "headerChips": [{"text": "road R1", "kind": "line"}, {"text": m["language"], "kind": "mono"}, {"text": f"{m['kind']} · {m['status']}", "kind": "ok" if m["status"] == "ready" else "warn"},
