...
export const COLLECTION_GUIDE: GuidePassage[] = [
 ...
 },
 {
  ...
  "kind": "page",
  "text": " | `prompt=none` on the authorization endpoint from a hidden frame, or a refresh token with `offline_access` in the scope | The served pages allow the hub to frame itself and the provider (`frame-src`, `frame-ancestors 'self'`) and to connect to the provider (`connect-src`), nothing else |\n| Sign-out | `end_session_endpoint` in discovery | The hub sends the person there and back to `HUB_WEB_OIDC_POST_LOGOUT_URI` |"
 },
 {
