 ...
 * the authority (specification §4.1, PLT-ID-1).
 */
...
export interface Principal {
  ...
  tenant: string;
  roles: Role[];
  /** The person's own autonomy ceiling; a consumer's ladder may be lower (spec §5.5). */
  ladder: Ladder;
  channel: Channel;
...
export interface Preferences {
  ...
  /** PLT-UI-15: self-declared assistive-technology preference, carried as an entitlement on every channel. */
  accessibility: boolean;
  /** PLT-CH-18: a person may decline AI assistance; routes to a person by default. */
  noAssistant: boolean;
