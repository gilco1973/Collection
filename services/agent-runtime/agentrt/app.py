...
FORBIDDEN_STOPS = ("taint.forbids_tier", "ladder.violation")   # a confirmed write refused for good; anything else may be confirmed again
BAD_BODY = object()  # `_body` answered the request already (4xx); distinct from any JSON value a body can carry


def make_handler(w, resource: str, max_runs: int = 2000, socket_timeout_s: float = 30.0):
        ...
        def do_POST(self):
            ...
            if self.path.startswith("/runs/") and self.path.endswith("/confirm"):
                ...
                try:
                    ...
                    # the confirmation was consumed but the write did not happen (the handler failed, a budget ran
                    # out): the same call is parked again in the record, so the person can confirm the same hash
                    repark(s, r["parked"])
...
def serve(w, host: str, port: int, resource: str, max_runs: int = 2000, socket_timeout_s: float = 30.0) -> ThreadingHTTPServer:
    """`socket_timeout_s`: a connection that sends nothing for that long is closed without an answer and holds no thread."""
    httpd = ThreadingHTTPServer((host, port), make_handler(w, resource, max_runs, socket_timeout_s))
