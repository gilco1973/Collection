...
| The hub (front end): pixel-identical to the design, real API mode, self-hosted fonts | DONE | `cd hub && pnpm verify`; a browser run against hub-api in `http` mode signed a component through the real flow | The production build with the bank's identity provider (`hub/.env.production`) and the redirect URI registered (identity engineer) |
...
| Packaging: containers non-root, read-only root, health checks, fail-closed entrypoints | WRITTEN | `services/*/deploy/Dockerfile`, `entrypoint.sh`; `deploy/compose.yaml` runs both locally with fakes | Base image digests pinned to the bank's approved image; images built in the bank's registry (integrations engineer) |
