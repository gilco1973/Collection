...
| Identity: RS256 through the JWKS, groups to roles, groups overage refused, mock refused in production; the browser path (PKCE, silent renew, sign-out, the refusals) proven against a stand-in OpenID Connect provider over TLS | DONE (against a stand-in provider) | `scripts/smoke-oidc.sh` in `scripts/verify.sh hub`: fourteen checks in a real Chromium against `hub/dist` served by hub-api with `HUB_AUTH=oidc`; `services/hub-api/tests/test_oidc.py` | Run once against the bank's provider in staging; the `groups` claim filtered to the hub's groups on both app registrations (identity engineer) |
...
| Offline delivery: a release bundle with the hub prebuilt and nothing to fetch; a delivery zip with the documents, the video and `INSTALL.md` | DONE | `scripts/bundle.sh`, `scripts/package.sh`; the bundle unpacked in a clean directory verifies its manifest and passes `scripts/verify.sh python` without network or Node | The bundle transferred through the bank's approved channel and its sha256 recorded (release manager) |
