...
- Agent: `GET /health` → `{"status":"ok","agent","env","build","identity","signing","engine","targets","records"}`: `identity`, `signing` and `engine` are the settings' values (`fake`/`oidc`, `local`/`kms`, `rules`/`bedrock`), `targets` maps each target to its connector's class name, and `records` is the chain's length after a verify. A fake value or class name in production is a misconfiguration.
...
| Back up the record | `python3 -m hubapi backup /var/hub/backup.db` and `python3 -m agentrt backup /var/agent/backup.db` inside the task (SQLite's online backup: consistent while serving), then copy the file off the volume; restore by stopping the task and putting the file back as the record (remove `<record>-wal` and `<record>-shm` first, see "The record is broken"). `hubapi backup` opens the source read-only and refuses a `HUB_DB` that is not an existing file, so a typo never backs up an empty record or migrates the live one. A record from a newer build refuses to open under an older one, and both `backup` commands refuse it with the same `record:` line (a copy the build cannot read is not a backup it could restore); restore before rolling back |
